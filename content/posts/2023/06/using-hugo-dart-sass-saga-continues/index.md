...
This return to more stable use of Dart Sass with Hugo could let you easily try out some (but **not** all[^SassCSSNesting]) features of [native CSS nesting](https://drafts.csswg.org/css-nesting-1/) while you're waiting for broader browser support; just put it in *.scss* files rather than *.css* files. Of the major browser platforms, Chrome and Safari already support native CSS nesting; and it [appears](https://caniuse.com/css-nesting) Firefox will [join them](https://bugzilla.mozilla.org/show_bug.cgi?id=1648037) in that regard within a few weeks, first behind a flag in [Firefox 115](https://whattrainisitnow.com/release/?version=115) and then by default in [Firefox 116](https://whattrainisitnow.com/release/?version=116). Then, when there's [enough browser support](https://caniuse.com/css-nesting) for you to feel comfortable in using the CSS as-is with no Dart Sass help, remove Dart Sass from the equation and adjust your styling as necessary to reconcile the differences[^SassCSSNesting] between Sass's nesting model and that of native CSS nesting.
