...
- **It should have decent contrast**. --- In other words, there should be enough difference between the color of the type and the color of the background.
	I don't mean stark black on stark white, or vice versa, because that's _too_ much contrast on a display --- especially on a super-high-contrast OLED display such as is becoming _de rigueur_ for [smartphones](https://www.oled-info.com/oled_devices/mobile_phones). UI experts have told me they suggest a combination of `#333` (really dark gray) on `#fff` (white).\
	\
