...
It was a different story when I kept mulling over the [second, somewhat trickier approach](/posts/2022/03/using-dart-sass-hugo-sequel/). It enables Hugo to use [Embedded Dart Sass from within the Dart Sass binary](https://github.com/sass/dart-sass#embedded-dart-sass), through shell scripting that tells one of three hosting vendors --- [Netlify](https://netlify.com), [Vercel](https://vercel.com), or [Cloudflare Pages](https://pages.cloudflare.com) --- to install the binary within the proper build-image `PATH`. As a result, the binary is "visible" to Hugo during the build, and all goes on smoothly from there.
...
```yaml{filename="netlify-deploy.yaml" bigdiv=true}
...
```yaml{filename="vercel-deploy.yaml" bigdiv=true}
...
```yaml{filename="cfp-deploy.yaml" bigdiv=true}
...
So, you may be wondering, does all this *truly* solve the problems I imagined for the [shell-scripting method](/posts/2022/03/using-dart-sass-hugo-sequel/)?
